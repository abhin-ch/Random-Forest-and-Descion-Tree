...
class RandomForest:
    ...
    def build(self, X, y):
        ...
        for tree_i in range(self.num_trees):
            ...
            sample = self.rng.permutation(np.arange(N))[:num_data_per_tree]
            feat_ids = self.rng.permutation(np.arange(D))[:num_features_per_tree]

            X_sub = X[sample,:]
            X_sub = X_sub[:, feat_ids]
            #print("This is my X_sub", len(X_sub))
            y_sub = y[sample]
            #print("This is my y_sub", len(y_sub))
            # ====================================================
